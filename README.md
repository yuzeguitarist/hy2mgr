...
8) **日志里出现 `accepting stream failed: timeout: no recent network activity`**
- 这通常是连接在**长时间无数据**时触发的空闲超时（常见于 NAT/移动网络）
- 若业务正常，可忽略；若频繁断开：
  - 客户端启用/调低心跳（keepalive）或保持少量周期性流量
  - 排查 UDP 被中间网络设备限速/丢弃（换端口或换网络）
  - 确认服务器与客户端系统时间同步
